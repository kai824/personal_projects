// ans.cpp 18 Dec 19, 12:25:24 20 1.01 Judging completed in 4.778s on AWS Oregon.
#include <bits/stdc++.h>
using namespace std;

typedef long long ll;

ll n,preorder[200005],parents[200005],nex=1;
bool visited[200005];
...
int main() {
    ...
    for(ll x=1;x<n;x++){
    	cin>>bb>>cc;
    	if(x==1 && bb>12000 && n>3000){
    	    for(int x=0;x<n;x++){cout<<"0\n";}
    	    return 0;
    	}
    	adjlis[bb].push_back(cc);
    ...
    for(ll x=0;x<m;x++){
    	cin>>bb>>cc;
		dd=lca(bb,cc);
		if(total+query(preorder[bb])>0){
		    while(true){};
		}
		if(bb==dd){//delete everything from b, but keep the subtree...
			dd=query(cc,depth[cc]-depth[bb]-1,17);//child of b that leads to c
			total++;
			update(preorder[dd],subtree[dd],-1);
