// ans.cpp 7 Aug 19, 08:03:15 100 1 Judging completed in 6.313s on AWS Oregon.
#include<bits/stdc++.h>
using namespace std;
...
inline void backtrack(int depth,ll rw, ll ld, ll rd){//depth, row, diagonals
	if(depth>n){
	    //if(rw!=full)return;
		cnt++;
		return;
...
int main(){
	...
	for(int x=0;x<25;x++){
	    cur=0;
	    for(int y=0;y<invalid[x].size();y++){
	        cur|=(invalid[x][y]);
	    }
	    complements[x]= ~cur;
	}
