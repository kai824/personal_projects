// ans.cpp 3 Feb 18, 15:34:46 100 0.88 Judging completed in 7.257s on AWS Oregon.
#include<bits/stdc++.h>
using namespace std;
...
void xupdate(ll row,ll s,ll e){
	...
	m->update(s,1,1,1e9+1);
	m->update(e+1,-1,1,1e9+1);
}
void yupdate(ll column,ll s,ll e){
    auto it=ynums.find(column);
	if(it==ynums.end()){
		m=new node();
		ynums[column]=m;
	}else{
...
ll xquery(ll row,ll column){
    m=xnums[row];
	if(m==NULL)return 0;
	return m->query(1,column,1,1e9+1);
}
ll yquery(ll row,ll column){
    m=ynums[column];
	if(m==NULL)return 0;
	return m->query(1,row,1,1e9+1);
}
int main(){
