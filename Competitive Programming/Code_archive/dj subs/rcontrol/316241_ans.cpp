// ans.cpp 14 Jan 18, 21:44:46 100 0.95 Judging completed in 6.608s on AWS Oregon.
#include<bits/stdc++.h>
using namespace std;
...
ll xquery(ll row,ll column){
	...
	return xnums[row]->query(1,column,1,1e9+1);
}
ll yquery(ll row,ll column){
	if(ynums[column]==NULL)return 0;
	return ynums[column]->query(1,row,1,1e9+1);
}
int main(){
