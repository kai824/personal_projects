// ans.cpp 14 Jan 18, 21:32:13 1 0.77 Judging completed in 6.092s on AWS Oregon.
#include<bits/stdc++.h>
using namespace std;
...
ll xquery(ll row,ll column){
	...
	return xnums[row]->query(1,column,1,2e9);
}
ll yquery(ll row,ll column){
	if(ynums[column]==NULL)return 0;
	return ynums[column]->query(1,row,1,2e9);
}
int main(){
