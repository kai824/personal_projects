// ans.cpp 3 Mar 20, 22:56:53 0 0.05 Judging completed in 3.883s on AWS Oregon.
#include "bits/stdc++.h"
using namespace std;
#define int uint16_t
#define bigbig int64_t
...
void brute_force(int num){//subsets of num...
	...
	for(int x=0;x<16;x++){
		if((1<<x)&num){//ooh, tea...
			ccc=ans.size();
			for(bigbig i=0;i<ccc;i++)ans.push_back(ans[i]+(1<<x));
		}
	}
}
...
inline void update1(int num,int32_t inc=1){
	...
	cnt1[num]+=inc;
	brute_force(num);
	for(bigbig i=1;i<ans.size();i++)cnt3[ans[i]]+=inc;
}
    ...
    while(op--){
        ...
        if(ss=="add"){
            ...
            if(c<=mid){
				...
				a=0;
				brute_force(int(~int(xx)));
				sort(ans.begin(),ans.end(),cmp);
				for(bigbig i=1;i<ans.size();i++){
					a+=cnt3[ans[i]];
					if(bit_cnt[ans[i]])a-=cnt3[ans[i]]*(bit_cnt[ans[i]]);//avoid repeated counting...
