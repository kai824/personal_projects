// ans.cpp 25 Jan 18, 10:26:50 0 0 Compilation failed on AWS Oregon.
/*

NOT DONE:
CHANGE THE UPDATE FUNCTION TO ADHERE TO THE CHANGE IN SPECIAL STUFF
CHANGE THE QUERY FUNCTION TO GIVE MORE INFORMATION
...
struct node{
	long long v=DEFAULT_VALUE,rs=DEFAULT_VALUE,ys=DEFAULT_VALUE;
	...
	void update(ll sx,ll sy,ll ex, ll ey,   ll x,ll y,ll add,ll special,bool colour){
		...
		if(x<=((sx+ex)/2)){//top
			if(y<=((sy+ey)/2)){//left
				...
				tl->update(sx,sy,((sx+ex)/2),((sy+ey)/2),x,y,add,special);
			}else{//right
				if(tr==NULL)tr=new node();
				tr->update(sx,((sy+ey)/2)+1,((sx+ex)/2),ey,x,y,add,special);
			}
		}else{//bottom
			if(y<=((sy+ey)/2)){//left
				if(bl==NULL)bl=new node();
				bl->update(((sx+ex)/2)+1,sy,ex,((sy+ey)/2),x,y,add,special);
			}else{//right
				if(br==NULL)br=new node();
				br->update(((sx+ex)/2)+1,((sy+ey)/2)+1,ex,ey,x,y,add,special);
			}
		}
		//update the value...
		v+=add;
			...
			if(it==mp.end()){
				...
				if((it->second%2)!=(e%2)){
					...
					if(e%2){
						...
						if(((b+c)%2)==1){
							stodd->update(1,1,R,C,b,c,-1,xxxxx);
						}else{
							steven->update(1,1,R,C,b,c,-1,xxxxx);
						}
					}else{
						//positive delta...
						if(((b+c)%2)==1){
...
/*
On each segment tree, add one more thing:number of red special nodes, number of yellow special nodes for update
for query, return 3 numbers->number of red, number of red special, number of yellow special
*/