// ans.cpp 25 Jan 18, 17:56:36 0 1.24 Judging completed in 5.442s on AWS Oregon.
#include<bits/stdc++.h>
using namespace std;
...
struct node{
	...
	pair<ll,pair<ll,ll> > query(ll sx,ll sy,ll ex,ll ey,     ll tx,ll ty,ll bx,ll by){
		if(tx<=sx && ex<=bx && ty<=sy && ey<=by)return make_pair(v,make_pair(rs,ys));
		pair<ll,pair<ll,ll> >ans=make_pair(0,make_pair(0,0));
		//tl
