// kindergarten.cpp 9 Aug 20, 15:37:50 0 2.13 Judging completed in 12.512s on AWS Oregon.
...
struct node{//segtree[i] stores the line you get if you end prev team at i, and take next team from i+1 onwards
  ...
  ll query(int a,int b,int x){
    ll ans=LLONG_MIN;
    if(a<=s && e<=b){
      for(int i=0;i<v.size();i++)ans=max(ans,v[i].first*x + v[i].second);
      return ans;
    }
