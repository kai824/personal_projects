// kindergarten.cpp 9 Aug 20, 16:00:35 41 1.01 Judging completed in 12.228s on AWS Oregon.
...
void init(int n, vector<int> T, vector<int> d, vector<int> H){
  ...
  for(ll i=2;i<=n;i++){
    ...
    if(leftt[i]==d[i-1]-1 && leftt[i]==1){//not possible to form teams, don't fake news to future dps
        dp[i]=-1;
        continue;
    }
