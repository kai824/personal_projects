// segmenttree.cpp 9 Dec 17, 10:55:10 100 0.52 Judging completed in 6.82s on AWS Oregon.
...
struct node{
	...
	void update(const int p,const int new_val,const int s,const int e){
	    if(p==s && p==e){
	        v=new_val;
	        return;
	    }
	...
	int query(const int a,const int b,const int s,const int e){
	    if(a<=s&&e<=b)return v;
	    int v_left,v_right;
