// pandagames.cpp 9 Aug 20, 18:26:45 20 0.15 Judging completed in 7.47s on AWS Oregon.
...
int game3(int N){//pyramid vector :)
  ...
  if(peak>1){
    ...
    while(lo<hi){
      mid=lo+((hi-lo)/2);
      if(look(mid,N+1)==mid)hi=mid;//v[n+1]<=v[mid]
      else lo=mid+1;//v[mid]<v[n+1]
    }
    //cerr<<"First possible result at "<<lo<<'\n';
    if(look(N+1,lo)==N+1)return lo;
  }
  lo=peak;hi=N;
  while(lo<hi){
    mid=hi-((hi-lo)/2);
    if(look(mid,N+1)==mid)lo=mid;//V[mid]>=V[n+1]
    else hi=mid-1;//V[N+1]>V[mid]
  }
