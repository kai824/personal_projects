// ans.cpp 2 Mar 18, 21:57:49 0 0.04 Judging completed in 5.039s on AWS Oregon.
#include <bits/stdc++.h>
using namespace std;
int dist[100005];
...
int main() {
    ...
    while(!bfs.empty()){
        ...
        for(int x=0;x<adjl[a].size();x++){
        	if(dist[adjl[a][x].first]!=0 && vis[adjl[a][x].first]==0){
            	if(adjl[a][x].first!=d)bfs2.push((adjl[a][x].first));
            	/*if(adjl[a][x].first==d){
            		cout<<"-2";
            		return 0;
            	}*/
            ...
            if(dist[a]+adjl[a][x].second>dist[adjl[a][x].first]){
                ...
                if(adjl[a][x].first!=d)bfs.push(adjl[a][x].first);
