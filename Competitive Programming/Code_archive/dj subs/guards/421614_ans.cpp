// ans.cpp 25 Nov 18, 19:04:44 0 0.17 Judging completed in 2.901s on AWS Oregon.
#include<bits/stdc++.h>
using namespace std;
typedef long long ll;
ll arr[2][8005],ps[8005],curr;
bool cur;
void dnc(ll l, ll r, ll s, ll e){
	if(l>=r)return;
	ll mid=(l+r)/2,best_it=s;
	arr[cur][mid]=LLONG_MAX;
	for(ll x=max(s,curr-1);x<=min(e,mid-1);x++){
	    if(arr[!cur][mid]+ ((ps[mid]-ps[x])*(mid-x))<arr[cur][mid]){
	        best_it=x;
	        arr[cur][mid]=arr[!cur][mid]+ ((ps[mid]-ps[x])*(mid-x));
	    }
	}
