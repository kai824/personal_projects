// dna.cpp 17 Nov 16, 15:00:34 0 0 Compilation failed on AWS Ireland.
#include "dna.h"
#include <iostream>
#include <vector>
using namespace std;

bool test(vector<int> v,string p){//v is the false stuff,p is the main input;
    for(int c=0;c<v.size();c++){
        if(p.substr(p.length()-v[c].length())==v)return false;
