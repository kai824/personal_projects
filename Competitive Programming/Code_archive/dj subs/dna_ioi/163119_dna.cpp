// dna.cpp 18 Nov 16, 21:16:01 100 0.52 Judging completed in 5.69s on AWS Ireland.
#include "dna.h"
#include <iostream>
#include <vector>
using namespace std;
...
string analyse(int n, int t){
    ...
    if(n==1){
        ...
        if(n>=10 && make_test("00000")){
            ans="00000";
        }else if(n>=10 && make_test("11111")){
            ans="11111";
        ...
        while(ans.length()<n){
            if(nm_back==false){
                if(nm_back2==false && test(i,(ans+"0"))) ans=ans+"0";
                else if(n-ans.length()>=2 && nm_front==false){
                    if(nm_back2==false && test(i,(ans+"10"))){
                        i.push_back(ans+"0");
                        ans=ans+"10";
           ...
           if(nm_front==false){
                if(nm_front2==false && test(i,("0"+ans))) ans="0"+ans;
                else if(n-ans.length()>=2 && nm_back==false){
                    if(nm_front2==false && test(i,("01"+ans))){
                        i.push_back("0"+ans);
                        ans="01"+ans;
                    }else if(nm_front2==false && n-ans.length()>=3 && test(i,("011"+ans))){
                        i.insert(i.end(),{("0"+ans),("01"+ans)});
                        ans="011"+ans;
                    }else if(nm_front2==false && n-ans.length()>=4 && test(i,("0111"+ans))){
                        i.insert(i.end(),{("0"+ans),("01"+ans),("011"+ans)});
                        ans="0111"+ans;
                    }else if(nm_front2==false && n-ans.length()>=5 && test(i,("01111"+ans))){
                        i.insert(i.end(),{("0"+ans),("01"+ans),("011"+ans),("0111"+ans)});
                        ans="01111"+ans;
                    }else if(nm_front2==false && n-ans.length()>=5 && test(i,("11111"+ans))){
                        i.insert(i.end(),{("0"+ans),("01"+ans),("011"+ans),("0111"+ans),("01111"+ans)});
                        ans="11111"+ans;
