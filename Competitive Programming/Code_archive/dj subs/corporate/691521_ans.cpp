// ans.cpp 1 Feb 20, 10:11:15 0 0 Compilation failed on AWS Oregon.
#include<bits/stdc++.h>
using namespace std;
	...
	for(ll x=1;x<=n;x++){
		...
		if(i-1>=1 && active[i-1]){
			...
			if(siz[i2]<siz[i3]){//iterate through the set at i2...
				...
				or(map<ll,ll>::iterator it=ss2[i3].begin();it!=ss2[i3].end();it++){
					if(ss[i2].find((it->first+arr[x].first)%m)!=ss[i2].end()){
						ans+=it->second*ss[i2][(it->first+m-arr[x].first)%m];
					}
				}
			}
			merge(i2,i3);
