// ans.cpp 1 Feb 20, 11:10:11 0 1.26 Judging completed in 5.551s on AWS Oregon.
#include<bits/stdc++.h>
using namespace std;
		...
		if(i-1>=1 && active[i-1]){
			...
			if(siz[i2]<siz[i3]){//iterate through the set at i2...
				for(map<ll,ll>::iterator it=ss[i2].begin();it!=ss[i2].end();it++){
					if(ss[i3].find((it->first+m-arr[x].first)%m)!=ss[i3].end()){
						ans+=it->second*ss[i3][(it->first+m-arr[x].first)%m];
					}
				}
			}else{//iterate through set at i3...
				for(map<ll,ll>::iterator it=ss[i3].begin();it!=ss[i3].end();it++){
					if(ss[i2].find((it->first+arr[x].first)%m)!=ss[i2].end()){
						ans+=it->second*ss[i2][(it->first+arr[x].first)%m];
					}
				}
			}
			merge(i2,i3);
