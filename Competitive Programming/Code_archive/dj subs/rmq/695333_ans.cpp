// ans.cpp 6 Feb 20, 22:39:27 30 0.1 Judging completed in 4.234s on AWS Oregon.
#include"bits/stdc++.h"
using namespace std;
...
struct node3{
	...
	node3(int ss,int ee){
		...
		if(s==e){
			...
			l=new node3(ss,(ss+ee)/2);
			r=new node3((ss+ee)/2+1,ee);
			v=min(l->v,r->v);
		}
	}
	void update(int p){
...
int main(){
	...
	root->get_arr(0);
	root3=new node3(0,n-1);
	for(int x=0;x<n;x++){
	    if(range[x].first>range[x].second){
			oops();
			return 0;
		}
		if(range[x].first==INT_MIN)continue;
		if(root3->query(range[x].first,range[x].second)>x){
		    //assert(false);
		    oops();
		    return 0;
		}
