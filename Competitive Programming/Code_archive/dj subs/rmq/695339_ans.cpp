// ans.cpp 6 Feb 20, 22:53:18 100 0.13 Judging completed in 5.546s on AWS Oregon.
#include"bits/stdc++.h"
using namespace std;
...
int main(){
	...
	for(int x=n-1;x>=0;x--){//do the stuff for x
		...
		while(arr[tmp]>x){
			if(unassigned.size()==0 || *unassigned.rbegin()<arr[tmp]){
				oops();
				return 0;
			}
			ans[tmp]=*unassigned.lower_bound(arr[tmp]);//must meet limit...
			arr[tmp]=-1;//give it a value...
			unassigned.erase(ans[tmp]);
			root2->update(tmp);
			tmp=root2->query(max(range[x].first,0),min(range[x].second,n-1));
		}
	...
	for(int x=0;x<n;x++){
		if(ans[x]==-1){
			if(unassigned.size()==0 || *unassigned.rbegin()<arr[x]){
				oops();
				return 0;
			}
			ans[x]=*unassigned.lower_bound(arr[x]);//must meet limit...
			unassigned.erase(ans[x]);
