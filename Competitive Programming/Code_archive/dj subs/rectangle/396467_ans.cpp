// ans.cpp 10 Oct 18, 18:21:21 100 0.31 Judging completed in 3.824s on AWS Oregon.
#include<bits/stdc++.h>
using namespace std;
#pragma O3
void main2(long long n){
    ios_base::sync_with_stdio(false);cin.tie(0);
	long long p1,p2,pp1,pp2,pp3;
	long long max_area=0,new_one;
	double diff1,diff2,gradient,len,c,d,xx;
	pair<double,double> coords[n];
	for(long long x=0;x<n;x++){
		cin>>coords[x].first>>coords[x].second;
	}
	vector<pair<pair<pair<double,double>,pair<double,double> >,pair<short int,short int> > >lines;
