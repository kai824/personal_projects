// ans.cpp 7 Mar 20, 10:58:11 40 0.52 Judging completed in 4.33s on AWS Oregon.
#include "bits/stdc++.h"
using namespace std;
...
void dfs(int node,int prev=-1){
	...
	for(int x=0;x<adjl[node].size();x++){
		...
		if(vis[adjl[node][x]]){
			if(depth[adjl[node][x]]<depth[node]-2)back=true;
			continue;
		}
    ...
    int tc,a,b,ans=0;
		...
		while(tc--){
			...
			for(int x=0;x<b_nodes.size();x++){
				a=dist1[b_nodes[x]];
				b=dist2[b_nodes[x]];//see who will reach there first...
