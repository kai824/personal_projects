// ans.cpp 7 Mar 20, 11:05:02 100 0.58 Judging completed in 4.639s on AWS Oregon.
#include "bits/stdc++.h"
using namespace std;
...
int dfs(int node,int prev=-1){
	...
	int mini=depth[node]+1;
	vis[node]=true;
	for(int x=0;x<adjl[node].size();x++){
		if(adjl[node][x]==prev)continue;
		if(vis[adjl[node][x]]){
			if(depth[adjl[node][x]]<depth[node]-2)mini=min(mini,depth[adjl[node][x]]);
			continue;
		}
		mini=min(mini,dfs(adjl[node][x],node));
	}
	if(mini<=depth[node])cycle[node]=true;
	return mini;
}
...
int main() {
    ...
    int tc,a,b,ans=0;
		...
		while(tc--){
			...
			for(int x=1;x<=n;x++){
				if(cycle[x]==false)continue;
				a=dist1[x];
				b=dist2[x];//see who will reach there first...
