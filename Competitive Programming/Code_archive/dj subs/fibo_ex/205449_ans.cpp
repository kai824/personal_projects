// ans.cpp 29 Apr 17, 21:06:59 0 0.2 Judging completed in 3.698s on AWS Oregon.
#include <bits/stdc++.h>
using namespace std;
pair<uint64_t,uint64_t> fibo(uint64_t a,uint64_t m){//returns make_pair(f(a)%m,f(a+1)%m);
	...
	if(a%2==0){
		...
		return(make_pair(mypair.second%m,(mypair.first+mypair.second)%m));
	}
}
int main() {
