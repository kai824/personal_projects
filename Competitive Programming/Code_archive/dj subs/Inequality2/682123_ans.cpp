// ans.cpp 18 Jan 20, 17:12:42 27 0.23 Judging completed in 4.912s on AWS Oregon.
#include<bits/stdc++.h>
using namespace std;
...
void main2(int n){
	...
	vector<pair<ll,ll> > v;//sliding window, maintain this to be decreasing...
	
	v.push_back(make_pair(INT_MAX,-1));
	
	for(int x=0;x<n;x++){
		while(v.size()>0 && v.back().first<=arr[x]+bb[x]){
			v.pop_back();
		}
		ans-=cnt[x]*arr[x];
		ans+=(arr[x]+bb[x])*(x-v.back().second);
		v.push_back(make_pair(arr[x]+bb[x],x));
		cout<<ans<<'\n';
	}
}
