// terrain.cpp 9 Aug 20, 17:07:02 0 2.03 Judging completed in 10.92s on AWS Oregon.
#include "terrain.h"
#include "bits/stdc++.h"
using namespace std;
...
int best_hike(int n, int e, vector<int> h, vector<int> d, vector<int> a, vector<int> b){
  ...
  for(int i=0;i<e;i++){
    ...
    if(d[a[i]-1]==d[b[i]-1]){//cos of the non-overlapping condition, can just assume same d will always go upwards...
      if(h[a[i]-1]<h[b[i]-1] || a[i]==1)adjl[a[i]].push_back(b[i]);
  ...
  sort(nodes.begin()+1,nodes.end());//ensure node 1 stays in front
  ...
  for(int i=0;i<hh.size();i++){
    ...
    for(int i=0;i<n;i++){
      node=nodes[i].second;
      if(dp[node]==INT_MAX)continue;//can't even reach this node smh
      if(h[node-1]<thres)continue;
      for(int j=0;j<adjl[node].size();j++){
        dp[adjl[node][j]]=min(dp[adjl[node][j]],max(dp[node],h[adjl[node][j]-1]) );
      }
    }
