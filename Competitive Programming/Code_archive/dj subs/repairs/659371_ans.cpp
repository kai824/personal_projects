// ans.cpp 21 Dec 19, 22:50:35 17 0.29 Judging completed in 5.687s on AWS Oregon.
#include<bits/stdc++.h>
using namespace std;
...
struct node{
	...
	node *l=NULL,*r=NULL;
	node(){}
	void sett(int s,int e,int a,int b,int v){
		//cout<<'s'<<s<<' '<<e<<' '<<a<<' '<<b<<' '<<v<<'\n';
		if(a<=s && e<=b){
			lazy=0;l_set=v;
			return;
	...
	void update(int s,int e,int a,int b,int c){
		//cout<<'u'<<a<<' '<<b<<' '<<c<<'\n';
		if(a<=s && e<=b){
			lazy+=c;
			return;
