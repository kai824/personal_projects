// ans.cpp 15 Feb 17, 20:06:41 0 0.04 Judging completed in 7.264s on AWS Ireland.
#include <bits/stdc++.h>
using namespace std;
struct connection{
...
int n;
pair<unsigned int,connection> adjmatrix[105][105],f;//adjmatrix shall be 1-indexed
int main() {
	f.first=INT_MAX;
	for(int x=0;x<105;x++){
		for(int y=0;y<105;y++){
    ...
    for(int x=0;x<q;x++){
    	cin>>b>>c;
    	if(adjmatrix[b][c].first==INT_MAX)cout<<-1<<endl;
