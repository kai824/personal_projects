// ans.cpp 19 Oct 19, 18:03:53 0 0.84 Judging completed in 5.133s on AWS Oregon.
#include "bits/stdc++.h"
using namespace std;
...
pii get_root(int a,int b){
    if(parent[a][b]==make_pair(-1,-1))return make_pair(a,b);
    parent[a][b]=get_root(a,b);
    return parent[a][b];
}
    ...
    for(int x=0;x<n;x++){
        for(int y=0;y<m;y++){
          ...
          for(int z=0;z<4;z++){
              ...
              if(invalid)continue;
              ans+=get_sizz(x+xx[z],y+yy[z]);
          }
          ans=max(ans,cur);
        }
    }
    cout<<ans+get_sizz(rain.first,rain.second);
    return 0;
}