// ans.cpp 7 Dec 19, 11:57:39 36 0.32 Judging completed in 5.141s on AWS Oregon.
#include <bits/stdc++.h>
using namespace std;
typedef pair<int,int> pii;
int parent[10005],mini[10005],maxi[10005];//min and max height of current container


set<pair<pii,int> >::iterator it,it2;
unordered_map<int,set<pair<pii,int> > > vp;//vertical edge
unordered_map<int,set<pair<pii,int> > > hp;//horizontal edge
...
void main2(int n,int h,int w){
	int a,b,c,d,o=-1;
	bool arr[h];
	for(int x=0;x<h;x++)arr[x]=false;
	for(int x=0;x<n;x++){
		cin>>a>>b>>c>>d;
		if(o==-1)o=b;
		else if(o!=b){
