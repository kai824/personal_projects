// ans.cpp 7 Jan 18, 16:11:05 0 1 Judging completed in 4.536s on AWS Oregon.
#include <iostream>
using namespace std;
...
long long mul(long long a,long long b){
    ...
    if(a<0)neg=!neg;
    if(b<0)neg=!neg;
    long long ans=0,xx=0,yy,bb;
    for(long long x=1;a;x=x<<1){
        if((a&x)==0){
            xx=add(xx,1);
            continue;
        }
        yy=0;
        bb=b;
        for(long long y=1;bb;y=y<<1){
            if((bb&y)==0){
                yy=add(yy,1);
                continue;
            }
            ans=add(ans,(1<<add(xx,yy)));
            yy=add(yy,1);
            bb^=y;
        }
        a^=x;
