// ans.cpp 22 Feb 20, 13:21:31 36 1.4 Judging completed in 5.757s on AWS Oregon.
#include"bits/stdc++.h"
using namespace std;
...
struct node{
  ...
  ll query(ll a,ll b){//left and right bounds...
    ...
    if(lazy2){
      if(l!=NULL)l->update2(1,e);
      if(r!=NULL)r->update2(1,e);
      lazy2=false;
    }
    if(b<=(s+e)/2){
  ...
  void update2(int a,int b){
    ...
    if(lazy2 && s<e){
      if(l!=NULL)l->update2(1,s);
      if(r!=NULL)r->update2(1,s);
      lazy2=false;
    }
    if(l==NULL)l=new node(s,(s+e)/2);
    if(r==NULL)r=new node((s+e)/2+1,e);
