// ans.cpp 22 Feb 20, 10:32:29 0 1.01 Judging completed in 4.675s on AWS Oregon.
#include"bits/stdc++.h"
using namespace std;
...
struct node{
  ...
  ll query(ll a,ll b){//left and right bounds...
    ...
    if(lazy)return walk_time;//the whole range will be the same...
    if(a<=s && e<=b){
...
int main(){
  ...
  while(d--){
    ...
    if(t==1){
      ...
      cout<<root->query(l,r)+(r-l+1)+abs(g-l)+abs(g-r)<<'\n';
    }else if(t==2){
      cin>>l>>r>>t;
      root->update(l,r,t);
    }else{
