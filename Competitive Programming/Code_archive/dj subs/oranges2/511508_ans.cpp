// ans.cpp 31 Mar 19, 21:24:16 0 0.57 Judging completed in 6.739s on AWS Oregon.
#include <bits/stdc++.h>
using namespace std;
//lca
...
void build(ll x,long long distance,ll d){//visits parents before children...
    visited[x]=true;
    depth[x]=d;
...
ll lca(ll a,ll b){
	...
	for(ll x=18;x>=0;x--){
		...
		if(c!=d){
			a=c;
			b=d;//make it count now
			ans=max(ans,max(val[x][a],val[x][b]));
		}
	}
