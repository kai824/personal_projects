// ans.cpp 29 Feb 20, 10:03:34 0 0.2 Judging completed in 5.605s on AWS Oregon.
#include"bits/stdc++.h"
using namespace std;
...
int32_t main(){
  ...
  for(int i=0;i<v.size();i++){
    if(v[i].first.second*v[i].second.first<=k){//cost * no. of times we can use
      cost-=v[i].first.first*v[i].second.first;//reward * "
      ...
      cost-=sum1*v[i].first.first;//reward * no. of packages...
      k-=v[i].first.second*sum1;

      sum1=k;//move some of the parcels forward by 1 step...
