// ans.cpp 29 Feb 20, 11:16:59 100 0.2 Judging completed in 4.569s on AWS Oregon.
#include"bits/stdc++.h"
using namespace std;
...
int32_t main(){
  ...
  //reward, cost(no. of ppl), no. of times we can take it, which side is it, x/y value
  ...
  for(int i=0;i<v.size();i++){
    ...
    if(v[i].first.second*v[i].second.first<=k){//cost * no. of times we can use
      ...
      cost-=sum1*v[i].first.first - sum1*(v[i].first.second-sum1);//no. of ppl * reward
      ...
      if(v[i].second.second.first==0 || v[i].second.second.first==2)to_change.first.second=to_change.first.first+1;
      else to_change.first.second=to_change.first.first-1;
