// ans.cpp 29 Feb 20, 10:44:24 14 0.19 Judging completed in 4.324s on AWS Oregon.
#include"bits/stdc++.h"
using namespace std;
  ...
  for(int i=0;i<v.size();i++){
    ...
    if(v[i].first.second*v[i].second.first<=k){//cost * no. of times we can use
      ...
      if(v[i].second.second.first<=1){
        to_change.first.first=xx[to_change.first.first];
        to_change.first.second=xx[to_change.first.second];
      }else{
        to_change.first.first=yy[to_change.first.first];
        to_change.first.second=yy[to_change.first.second];
      }
