// elephants.cpp 13 Aug 20, 21:58:09 100 6.98 Judging completed in 14.512s on AWS Oregon.
#include "elephants.h"
#include "bits/stdc++.h"
using namespace std;
...
inline void adjust(int x){//adjust bucket x, assuming stuff in there is alr sorted...
  rptr=pos[x].size()-1;
...
int update(int i, int y){
  ...
  if(ind==0){//rebuild...
    ...
    arr[i]=y;
    for(int x=0;x<=m;x++){
      if(x==m || arr[i]<=pos[x].back().first){
          a=x;//insert into this bucket...
          break;
      }
    }
