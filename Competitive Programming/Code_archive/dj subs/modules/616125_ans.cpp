// ans.cpp 25 Nov 19, 18:25:49 11 3.02 Judging completed in 8.228s on AWS Oregon.
#include <bits/stdc++.h>
using namespace std;
...
bool ok(int a){//checks if satisfaction of a can be achieved
    ...
    for(int x=0;x<n;x++){
        ...
        if(it<0){
            v.push_back(make_pair(mods[x].first.second,mods[x].second));
            continue;
        }
        lo=0;hi=v.size()-1;
        while(lo<hi){
        ...
        v.push_back(make_pair(mods[x].first.second,mods[x].second+v[lo].second));
