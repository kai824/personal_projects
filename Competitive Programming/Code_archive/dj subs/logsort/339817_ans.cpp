// ans.cpp 18 Mar 18, 20:18:51 100 0 Judging completed in 2.852s on AWS Oregon.
#include <iostream>
...
// Count sort of arr[].
void countSort(int arr[], int n, int expi)
{
	...
	for (i = n - 1; i >= 0; i--)
	{
		output[count[(arr[i] / expi) % 10] - 1] = arr[i];
		count[(arr[i] / expi) % 10]--;
	}
...
void radixsort(int arr[], int n)
{
	...
	for (expi = 1; m/expi > 0; expi *= 10)
		countSort(arr, n, expi);
}
