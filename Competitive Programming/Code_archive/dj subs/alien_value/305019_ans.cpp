// ans.cpp 25 Dec 17, 16:30:18 25 1 Judging completed in 7.061s on AWS Oregon.
#include <bits/stdc++.h>
using namespace std;
...
ll sumi(string s,string e){
	if(( ((s[s.length()-1]-'0')+(e[e.length()-1]-'0'))%2)==0){
		...
		return((mod(half(sum(s,e)),m)*mod(sum(subtract(e,s),"1"),m))%m);
	}else{
		//return(((((s+e)%m)*(((e-s+1)/2)%m)))%m);
		return((mod(sum(s,e),m)*mod(half(sum(subtract(e,s),"1")),m))%m);
	}
}
int main(){
    ...
    string p,q;
    cin>>p>>q;
    cout<<(ipow(2,mod(subtract(q,p),(m-1)))*( sumi(p,q)%m))%m;
    return 0;
}