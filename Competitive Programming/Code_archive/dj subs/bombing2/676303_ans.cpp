// ans.cpp 10 Jan 20, 12:06:20 15 0.15 Judging completed in 5.227s on AWS Oregon.
#include<bits/stdc++.h>
using namespace std;
...
int dfs(int node,int prev,int d){//returns the shallowest backedge...
	depth[node]=d;
	vis[node]=true;
	int shallow,local,cnt=0;//shallowest backedge...
	shallow=local=INT_MAX;
	for(int x=0;x<adjl[node].size();x++){
		if(prev==adjl[node][x])continue;
	    if(vis[adjl[node][x]]){
	        local=min(local,depth[ adjl[node][x] ]);
	    }
	}
	for(int x=0;x<adjl[node].size();x++){
