// dreaming.cpp 31 Jan 20, 21:06:16 18 0.12 Judging completed in 5.858s on AWS Oregon.
#include<bits/stdc++.h>
#include "dreaming.h"
using namespace std;
...
void dfs(int node,int dd=0,int prev=-1){
	dist[node]=dd;
	parent[node]=prev;
	if(dd>=max_dist){
		max_dist=dd;
...
int find_diameter(int node){//actually returns radius
	...
	dfs(end1,0,-1);
	end2=deepest;
	while(dist[parent[end2]]*2>max_dist){
		end2=parent[end2];
	}
	max_dist=0;dfs(parent[end2]);
	end1=max_dist;
	max_dist=0;dfs(end2);
	end2=max_dist;
