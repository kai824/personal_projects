// dreaming.cpp 26 Jun 20, 11:31:25 0 0 Compilation failed on AWS Oregon.
#include<bits/stdc++.h>
#include "dreaming.h"
using namespace std;
...
int find_diameter(int node){//actually returns radius
	...
	return min(max(max_dist-dist[parent[deepest]],dist[parent[deepest]]),max(dist[deepest],max_dist-dist[depest]));
}
