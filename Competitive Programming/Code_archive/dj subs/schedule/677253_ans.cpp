// ans.cpp 11 Jan 20, 17:01:18 0 0 Compilation failed on AWS Oregon.
#include "bits/stdc++.h"
using namespace std;
...
int main() {
    ...
    for(ll x=0;x<n;x++){
        it=upper_bound(v.begin(),v.end(),make_pair(m[x].first.first,-1))-v.begin();
				if(it==n || v[it].first>=m[x].first.first){
				    it--;
				}
				if(it<0)continue;
