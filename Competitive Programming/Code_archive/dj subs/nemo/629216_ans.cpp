// ans.cpp 4 Dec 19, 16:16:18 0 0 Compilation failed on AWS Oregon.
#include <bits/stdc++.h>
using namespace std;
int parent[50000];
int getr(int x){
...
int main() {
    ...
    for(int x=0;x<q;x++){
        broke=false;
        while(arr[x].first.first>=edges[it].first.first){
    ...
    for(int x=pause;x<q;x++){
        broke=false;
        while(arr[x].first.first>=edges[it].first.second){
            merge(edges[it].second.first);
            it++;
            if(it>=m){
                broke=true;break;
            }
