// ballgame.cpp 31 Jan 19, 22:16:39 0 0.59 Judging completed in 5.141s on AWS Oregon.
#include<bits/stdc++.h>
using namespace std;
...
inline void update(ll p,ll v){
    if(p==0)while(true){};
    v%=1000000007;
    for(;p<=1500000;p+=ls(p)){
        ft[p]+=v;
        ft[p]%=1000000007;
    }
}
inline ll query(ll s, ll e){
    if(s==0 || e==0)while(true){};
    ll ans=0;
    for(;e;e-=ls(e)){
        ans+=ft[e];
        ans%=1000000007;
...
int ballgame(int n, int l, int d, int h[], int c[]) {
    ...
    for(ll x=1;x<n;x++){
        a=query(max(lr[ppl[x].second].second.first,0LL),min(lr[ppl[x].second].second.second,dis));
