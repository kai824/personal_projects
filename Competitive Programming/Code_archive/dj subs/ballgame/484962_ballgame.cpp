// ballgame.cpp 31 Jan 19, 22:23:34 100 0.94 Judging completed in 6.396s on AWS Oregon.
#include<bits/stdc++.h>
using namespace std;
typedef int ll;
ll ft[2000000];
...
inline void update(ll p,ll v){
    p++;
    v%=1000000007;
    for(;p<=1600000;p+=ls(p)){
        ft[p]+=v;
        ft[p]%=1000000007;
    }
}
inline ll query(ll s, ll e){s++;e++;
    ll ans=0;
    for(;e;e-=ls(e)){
        ans+=ft[e];
        ans%=1000000007;
...
int ballgame(int n, int l, int d, int h[], int c[]) {
    ...
    pair<ll,pair<ll,ll> > lr[n];//middle, left, right
    //stores discretised, left, middle and right values
    pair<ll,pair<ll,char> > red[3*n];//for discretisation
    int ans=0,a;
	ll dis=1;
    for(ll x=0;x<n;x++){
    ...
    for(ll x=1;x<n;x++){
        a=query(max(lr[ppl[x].second].second.first,1),min(lr[ppl[x].second].second.second,dis));
