// ans.cpp 7 Apr 20, 08:50:10 0 0.15 Judging completed in 5.41s on AWS Oregon.
#include"bits/stdc++.h"
using namespace std;
...
void dfs(int node,int prev=-1){
  ...
  for(int x=0;x<adjl[node].size()-1;x++){
    if(adjl[node][x]==prev){
      swap(adjl[node][x],adjl[node].back());
    }
  }
  if(prev!=-1)adjl[node].pop_back();//only keep the children...
...
int32_t main(){
    ...
    while(q--){
      ...
      if(po[b]<=po[a] && po[a]<=st[b]){//b is ancestor of a...
        if(adjl[b].size()==0){
