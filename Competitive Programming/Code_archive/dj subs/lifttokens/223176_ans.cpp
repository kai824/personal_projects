// ans.cpp 17 Oct 17, 22:36:02 50 0.01 Judging completed in 4.92s on AWS Oregon.
#include <bits/stdc++.h>
using namespace std;
int main() {
  long long n=10,v,i,i2;
  cin>>v;
  long long coins[10]={1,5,10,17,50,100,500,1000,5000,10000};
  long long amt[v+1];
  for(i=0;i<=v;i++){
    amt[i]=1000000000000000000;
  }
  amt[0]=0;
  for(i=0;i<n;i++){
    for(i2=coins[i];i2<=v;i2++){
      amt[i2]=min(amt[i2-coins[i]]+1,amt[i2]);
      //cout<<amt[i2]+1<<" "<<i<<" "<<i2<<" "<<amt[i2+coins[i]]<<endl;
    }
  }
  //for(i=0;i<v+1;i++)cout<<amt[i]<<endl;
  if(amt[v]==1000000000000000000)cout<<"-1";
  else cout<<amt[v];
}