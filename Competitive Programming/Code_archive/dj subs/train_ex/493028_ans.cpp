// ans.cpp 18 Feb 19, 10:53:28 0 0.05 Judging completed in 5.93s on AWS Oregon.
#include<bits/stdc++.h>
using namespace std;
typedef pair<int,int> pii;
set<pair<double,pii> >lines;
set<pair<double,pii> >::iterator it,it2;
//(x-a)^2+h each stores x-coord of intersect with prev,a,h
double dd(double a,double b){//double divide
	double c=a,d=b;
	return(c/d);
}
void add_line(double a,double h){//more like curve
	it=lines.upper_bound(make_pair(a,make_pair(0,0)));
	if(it==lines.end()){//no next line to update
		lines.insert(make_pair(a,make_pair(a,h)));
	}else{
		double i1=dd( a+it->second.first+dd((h-it->second.second),(a-it->second.first)),2),i2;//new line intersection with next line
		while(true){
			//check if need to remove line at it
			i1=dd( a+it->second.first+dd((h-it->second.second),(a-it->second.first)),2);
			it++;
			if(it==lines.end()){
				it--;break;
			}
			i2=dd( a+it->second.first+dd((h-it->second.second),(a-it->second.first)),2);
			if(i2<=i1){
				it2=it;it2--;
				lines.erase(it2);//so now it pointing to next one
			}else{
				it--;break;
			}
		}
		//it should point to next one by now
		lines.insert(make_pair(dd( a+it->second.first+
		dd((h-it->second.second),(a-it->second.first))
		,2),it->second));//need to update intersect, since it's now with current line
		lines.erase(it);
		lines.insert(make_pair(a,make_pair(a,h)));
	}
}
int query(int x){//querying min value based on x
	it=lines.upper_bound(make_pair(x,make_pair(0,0)));//find first one larger than x
	it--;
	return((x- it->second.first)*(x- it->second.first)+it->second.second);
}
bool cmp(pii a,pii b){
	if(a.first==b.first){
		return(a.second<b.second);//the earlier one first
	}else return(a.first>b.first);//taller one first
}
int dist[50005];
int main(){
	/*//lines.insert(make_pair(0,make_pair(0,0)));
	//lines.insert(make_pair(1,make_pair(1,1)));
	//lines.insert(make_pair(3,make_pair(3,5)));
	add_line(0,0);
	add_line(1,1);
	add_line(3,5);
	cout<<query(2)<<'\n';
	add_line(2,2);
	it=lines.begin();
	while(it!=lines.end()){
		cout<<it->first<<' '<<it->second.first<<' '<<it->second.second<<'\n';
		it++;
	}
	return 0;*/
	ios_base::sync_with_stdio(false);cin.tie(0);
	int n;
	cin>>n;
	pii arr[n];
	dist[0]=0;
	for(int x=0;x<n;x++){
		cin>>arr[x].first;
		arr[x].second=x+1;//the a in the line
	}
	lines.insert(make_pair(0,make_pair(0,0)));
	sort(arr,arr+n,cmp);
	for(int x=0;x<n;x++){
		dist[arr[x].second]=query(arr[x].second);
		//since arr[x].second is like the x variable
		add_line(arr[x].second,dist[arr[x].second]);//a, h(constant)
	}
	cout<<query(n+1);/*<<'\n';;
	it=lines.begin();
	while(it!=lines.end()){
		cout<<it->first<<' '<<it->second.first<<' '<<it->second.second<<'\n';
		it++;
	}*/
	return 0;
}