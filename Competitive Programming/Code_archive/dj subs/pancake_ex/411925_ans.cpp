// ans.cpp 13 Nov 18, 22:03:44 80 2.83 Judging completed in 5.717s on AWS Oregon.
#include<bits/stdc++.h>
using namespace std;
#pragma O3
...
int main(){
	...
	for(int x=1;x<=10;x++){
		...
		while(!bfs.empty()){
			...
			for(int y=2;y<=x;y++){
				...
				if(m[x].find(convert(start))==m[x].end()){
					//cout<<"k\n";
					m[x][convert(start)]=bfs.front().second.first+1;
	...
	while(tc--){
		...
		bfs.push(make_pair(start,make_pair(0,0)));
		while(!bfs.empty()){
			if(m[n].find(convert(bfs.front().first))!=m[n].end()){//exists...
				ans=(m[n][convert(bfs.front().first)]+bfs.front().second.first);
				break;
			}
			for(int y=2;y<=n;y++){
				if(y==bfs.front().second.second)continue;
