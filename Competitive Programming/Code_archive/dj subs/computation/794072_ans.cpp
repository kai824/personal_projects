// ans.cpp 24 Nov 20, 20:18:54 0 17 Judging completed in 24.457s on AWS Oregon.
#include <bits/stdc++.h>
using namespace std;
...
#define process(x) (ll)floor(log2(x) )
...
int main() {
	...
	for(int i=1;i<n;i++){
		...
		for(int x=0;x<16;x++)arr[k][x]=(arr[j][x]+arr[k][x])%1000000007;
		for(int x=0;x<16;x++)arr[k][x]=(arr[k][x]*arr[k][x])%1000000007;
