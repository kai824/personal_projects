// ans.cpp 26 Nov 20, 00:01:53 100 8.97 Judging completed in 14.976s on AWS Oregon.
#include <bits/stdc++.h>
using namespace std;
...
int main() {
	...
	for(int i=1;i<n;i++){
		...
		for(int x=0;x<16;x++)arr[j][x] %= 1000000007ll;
		
		for(int x=0;x<16;x++)arr[j][x] *= process(arr[j][x]);
		for(int x=0;x<16;x++)arr[j][x] %= 1000000007ll;

		for(int x=0;x<16;x++)arr[k][x]+=arr[j][x];
		for(int x=0;x<16;x++)arr[k][x] %= 1000000007ll;
