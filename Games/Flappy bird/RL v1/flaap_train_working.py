#changes from v1: use replay memory of size 100k states (about 1000 games...)
...
total_gen=2500
BATCH_SIZE=50
REPLAY_SIZE=100000#size of memory, basically number of states you remember...
f=open('cache.dat','rb')#stored as list/deque of ((state,action), (next_state,action) )
memory=dq(pickle.load(f) )
f.close()
...
exploration_rate=1 - (gen_no/total_gen)#also known as epilson
discount=0.99
    ...
    if score.over==True:
        ...
        for i in range(len(player.states)):#can prolly instead do random sampling from memory, but wtv
            if i+1<len(player.states):
                memory.append((player.states[i],player.states[i+1]) )
            else:
                memory.append((player.states[i],None) )
        while len(memory)>REPLAY_SIZE:
            memory.popleft()
