...
try:
    ...
    networks=[]
    for x in range(20):
        networks.append(enn.NeuralNetwork(2,4))
...
class Bird:
    ...
    def decide(self,obstacles,network):#take list of obstacles, get state, feed to network, decide on jump...
        ...
        while len(state)<4: state.append(0)
        #print(state)
        try:
...
class Obstacle:
    def __init__(self,canvas,player,score):
        ...
        self.lo=self.canvas.create_rectangle(700,height+100-30,740,400,fill='#228b22',outline='#228b22')
        self.passed=False
        self.i=False
        self.score=score
...
for gen in range(50000):#15 generations
    print(gen)
    fitnesses=[]
    ...
    for i in range(5):#reproduce 5...
        ...
        if enn.equal(fitnesses[a][1],fitnesses[b][1]):
