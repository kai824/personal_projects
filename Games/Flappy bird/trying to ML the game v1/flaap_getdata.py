...
class Bird:
    def __init__(self,canvas,colour,score,prev_states=None):
        self.speed=0.1
        ...
        try:
            ...
            self.states=[]#stores the surviving states...
    def nn(self,evt):
        if self.n==False:
            self.n=True
        else:
...
while True:
    if player.quit==True:
        #save the states...
        f=open('train_states.dat','wb')
        pickle.dump(player.states,f)
        f.close()
        canvas.destroy()
        tk.destroy()
        break
    ...
    if score.over==True:
        ...
        while len(obstacles)>0:
            ...
            del obstacles[0]
        score=Score(canvas,'black')
        player=Bird(canvas,'yellow',score,prev_states=player.states)#bring old states to the new bird...
        canvas.tag_raise(score.id)
        obstacles=[]
        cnt=40
    time.sleep(0.01)
