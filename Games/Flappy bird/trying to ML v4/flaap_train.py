...
try:
    ...
    networks=[]
    for x in range(15):
        networks.append(enn.NeuralNetwork(1,7))
        #2 hidden layers, 6 inputs:
        #velocity, dist to next pipe (Dx), dist to bottom pipe, dist to 2nd bottom pipe
        #dist to bottom pipe/Dx, dist to 2nd bottom pipe/Dx, constant (so that they can be added...)
...
class Bird:
    ...
    def decide(self,obstacles,network):#take list of obstacles, get state, feed to network, decide on jump...
        ...
        for o in obstacles:
            ...
            if 40<loc[2]:#not passed yet...
                if len(state)==1:
                    state.append(loc[2]-40)#x difference
                state.append(loc[1]-((plc[1]+plc[3])/2) )#y difference
                if len(state)>=4:
                    break#take next 2 obstacles...
        while len(state)<4:
            if len(state)==1:
                state.append(300)#still very far away...
            else:
                state.append(0)
        state.append(state[2]/state[1])
        state.append(state[3]/state[1])
        #print(state)
        #if len(obstacles)>0:
        #    return (state[2]-state[0]<=10)
        try:
            state.append(1)#add a constant for fun...
...
delta=30#in the actual thing, it's more of -min(30,score)
class Obstacle:
    def __init__(self,canvas,player,score):
        ...
        self.lo=self.canvas.create_rectangle(700,height+100+delta,740,400,fill='#228b22',outline='#228b22')
...
for gen in range(1000):#15 generations
    ...
    for net in networks[len(prev_results):] :
        fitness_val=0
        for seed_value in [100,200,300,400,500,600,700,800,900,1000]:
            #play with the net...
            ...
            random.seed(seed_value)#so that generate same obstacles...
            while True:
