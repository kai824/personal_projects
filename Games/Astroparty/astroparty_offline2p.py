    ...
    def __init__(self,canvas,start='#ffff00',end='#ff0000'):#the main stuff, plus collecting input
        ...
        for x in range(0,500):
            ...
            if (x%2)==0:
                tk.update()
                if (x%10)==0:
                    time.sleep(max(0,0.02-(time.time()-prev)))
                    sumi[0]+=(time.time()-prev)
                    sumi[1]+=1
                    prev=time.time()
            for y in range(3):
    ...
    def restart(self,start='#ffff00',end='#ff0000'):
        ...
        for x in range(0,500):
            ...
            if (x%2)==0:
                tk.update()
                if (x%10)==0:
                    time.sleep(max(0,0.1-(time.time()-prev)))
                    sumi[0]+=(time.time()-prev)
                    sumi[1]+=1
                    prev=time.time()
            for y in range(3):
    ...
    def press(self,evt):
        ...
        if e in self.dict:
            ...
            if 0.01<delay and delay<0.15 and self.dict[e][2]<0.1:#ensure previous press was quick press
                ...
                if time.time()-self.dict[e][1]<0.01:#questionable
    ...
    def mainloop(self):
        ...
        while True:
            ...
            if 'q' in self.dict:
                ...
                break
            tk.update()
            time.sleep(max(0.017-time.time()+start,0))
            durations.append(time.time()-start)
            sumi[0]+=time.time()-start
            sumi[1]+=1
            sumi[2]=max(sumi[2],time.time()-start)
            sumi[3]=min(sumi[3],time.time()-start)
tk=Tk()
tk.title('Astroparty')
tk.resizable(0,0)
